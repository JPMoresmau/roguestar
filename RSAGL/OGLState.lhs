...
\texttt{OGLLightSourceState} represents the OpenGL properties of a specific light source.

\begin{code}
data OGLLightSourceState = OGLLightSourceState
...
\texttt{OGLPerObjectState} represents the OpenGL state variables that describe how a particular surface should be rendered.

\begin{code}
data OGLPerSurfaceState = OGLPerSurfaceState { 
...
\texttt{OGLPerSceneState} represents the OpenGL state variables that describe how a particular frame of animation should be rendered.

\begin{code}
data OGLPerSceneState = OGLPerSceneState {
    ogl_background_color :: Color4 GLfloat,
    ogl_depth_func :: Maybe ComparisonFunction,
    ogl_light_model_ambient :: Color4 GLfloat,
    ogl_lights :: [OGLLightSourceState],
    ogl_fov_degrees :: GLdouble,
    ogl_near_plane :: GLdouble,
    ogl_far_plane :: GLdouble,
    ogl_fog :: Capability,
    ogl_fog_mode :: FodMode }

ogl_bare_bones_scene_configuration :: OGLStateConfiguration
ogl_bare_bones_scene_configuration = OGLStateConfiguratiion {
    ogl_background_color = Color4 0 0 0 0,
    ogl_depth_func = Just Lequal,
    ogl_light_model_ambient = Color4 0 0 0 1,
    ogl_lights = [],
    ogl_fov_degrees = 60, -- completely arbitrary value
    ogl_near_plane = 1,   -- completely arbitrary value
    ogl_far_plane = 10,   -- completely arbitrary value
    ogl_fog = Disabled,
...
-- |
-- Sets all the opengl state variables specified in the given OGLStateConfiguration.
-- You need to loadIdentity and set the camera before calling this, unless you
-- want the lights to be in eye coordinates (in which case you should load identity,
-- call this, and then set the camera.
--
