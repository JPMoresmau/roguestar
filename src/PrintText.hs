...
data PrintTextObject = PrintTextObject (IORef PrintTextData) (Chan B.ByteString)
...
printText :: PrintTextObject -> TextType -> B.ByteString -> IO ()
...
pullInputBuffer (PrintTextObject pto chan) =
    do e <- isEmptyChan chan
       when (not e) $
           do r <- readChan chan
              modifyIORef pto $ \print_text -> print_text { text_input_buffer = text_input_buffer print_text `B.append` r }
              return ()
...
clearInputBuffer (PrintTextObject pto _) = modifyIORef pto $ \ptd -> ptd { text_input_buffer = B.empty }
       ...
       let lines_to_print = restrictLines max_characters_height max_characters_width $ 
			    (case (text_output_mode ptd) of
	        		...
	        		Unlimited -> (text_output_buffer ptd)) ++
                            (if B.length (text_input_buffer ptd) > 0 || text_output_mode ptd /= PrintTextData.Disabled 
			        then [(Query,"> " `B.append` (text_input_buffer ptd))] 
			        else [])
	   actual_width_pixels = font_width_pixels * (maximum $ map (B.length . snd) lines_to_print)
