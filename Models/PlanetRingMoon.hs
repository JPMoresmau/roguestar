...
planet_ring_moon = model $
    do model $ do disc 0.75 1.0
                  ...
                  bumps $ waves 0.5 0.02
       model $ do sphere (Point3D 0 0 0) 0.65
                  let land_vs_water land water = pattern (cloudy 25 0.5) [(0,water),(0.45,water),(0.55,land),(1,land)]
                  let grass_and_mountains = pattern (cloudy 81 0.25) [(0.2,pure slate_gray),(0.3,pure forest_green)]
                  let land_and_water = land_vs_water grass_and_mountains (pure blue)
                  let cities bright dark = land_vs_water (pattern (cloudy 5 0.1) [(0.0,bright),(0.3,dark)]) (dark)
                  let planet_surface = pattern (gradient (Point3D 0 0 0) (Vector3D 0 0.65 0)) [(-0.9,pure white),(-0.85,land_and_water),(0.85,land_and_water),(0.9,pure white)]
